...
const CoursesStagesSection = React.forwardRef(({
  ...
  useEffect(() => {
    ...
    if (activeCourse !== null) {
      setFilteredLessons(
        lessons
          .filter(lesson => activeCourse.toLowerCase().includes(lesson.lekcjaPoziom))
  ...
  return (
    <StyledWrapper ref={ref} padding="0" bg="light">
      <AnimatePresence>
        {activeCourse === null ? (
          <StyledContentWrapper key="content" variants={fadeOutAnimation} initial="hidden" animate="show" exit="exit" padding="90px 107px 110px 102px">
